...
tail -f "$FILE" | jq -r '
def ar:
  (.assistant_response // empty) as $x
  | if ($x|type)=="string" then ($x|fromjson? // {"_raw":$x}) else $x end;

select(.assistant_response!=null) |
"\(.timestamp_utc) call=\(.call_id) role=\(.role) backend=\(.final_backend // "NA") " +
(if (ar|type)=="object"
 then "decision=" + (ar.decision // ("candidate=" + ((ar.candidate_index // "NA")|tostring))) +
      " reason=" + ((ar.reason // "")|gsub("[\r\n\t]+";" ")|.[0:180])
 else ((ar|tostring)|gsub("[\r\n\t]+";" ")|.[0:180])
 end)
'
...
salloc -A gts-sn73 -p gpu-rtx60 --gres=gpu:rtx_6000:1 --cpus-per-task=6 --mem=32G --time=01:00:00
srun --pty bash -l

...
class PhysicsFilter:
    ...
    - Total column count: 4-16 columns reasonable
    ...
    def apply_total_columns_constraint(self, nc: List[int]) -> Tuple[bool, Optional[str]]:
        """
        Check if total column count is reasonable.

        Too few columns (< 4) limits separation capability.
        Too many columns (> 16) increases complexity and solvent consumption.
        ...
        """
        total = sum(nc)
        min_cols, max_cols = 4, 16

        if total < min_cols:
            return False, f"Too few total columns ({total} < {min_cols})"
        if total > max_cols:
            return False, f"Too many total columns ({total} > {max_cols})"
...
def filter_and_rank_lhs_configs(
    ...
    """
    physics_filter = PhysicsFilter(strict=strict)
    scorer = ConfigScorer()
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("Physics Filter - Test with LHS Configs")
    print("=" * 70)
    print()

    # Generate LHS configs
    lhs_configs = generate_lhs_configs(60, seed=42)
    print(f"Generated {len(lhs_configs)} LHS configurations")
    print()

    # Filter and rank
    result = filter_and_rank_lhs_configs(lhs_configs, n_keep=40, strict=False)
